...
pane of a terminal multiplexer pane, relying on a sutiable terminal emulator's
...
* `only_write_files`: If true, write image files to `tempdir`, but to not
...
overrides the keyword arguments original given in
...
deactivated system, or a warning if the current display is not managed by `MultiplexerPaneDisplay`.
"""
